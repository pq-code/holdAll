...
<template>
  ...
  >
    ...
    >
     <div class="side-head-li" v-if="sideHeadLi">
        <div @click="lengthWidthFn('head')" style="cursor: pointer">
          <i class="iconfont icon-return"></i>
        </div>
        <!-- <slot name="sideHeadLi"></slot> -->
      </div>
      <div class="side-body-li" v-for="(item, index) in dataList" :key="`${item}-${index}`">
        <button class="button" @click="lengthWidthFn(item)" :style="{ 'justify-content': !lengthWidth ? 'flex-start' : 'center' }">
          <i :class="['iconfont', item.icon]"></i>
          <div v-if="!lengthWidth" style="margin-left: 10px">
            {{ item.title }}
          </div>
          <div class="button-dropDown" @click="lengthWidthFn(item)" v-if="item.options && item.options.length > 1 && !lengthWidth">
  ...
  <div class="p-side" @mouseleave="mouseleaveFn" @mouseenter="mouseenterFn">
    <div class="side-ul">
      <div class="side-head-li" v-if="sideHeadLi">
        <div v-if="!lengthWidth && foldSwitch" @click="lengthWidthFn('head')" style="cursor: pointer">
      ...
      <div class="side-body-li" v-else v-for="(item, index) in dataList" :key="`${item}-${index}`">
        <button class="button" @click="lengthWidthFn(item)" :style="{ 'justify-content': lengthWidth ? 'flex-start' : 'center' }">
          <i :class="['iconfont', item.icon]"></i>
          <div v-if="lengthWidth" style="margin-left: 10px">
            {{ item.title }}
